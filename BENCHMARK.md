...
c=50:
  无连接池:  QPS=1126.1, P99=48.7ms
  有连接池:  QPS=1126.1, P99=49.3ms
  → 无差异，瓶颈在 MySQL 单次查询时间，不在并发连接数

c=200:
  无连接池:  QPS=4380.6, P99=51.5ms
  有连接池:  QPS=4213.2, P99=55.2ms
  → 连接池反而略慢（连接池管理开销），但差异 <4%

c=500:
  无连接池:  ❌ 崩溃
  有连接池:  QPS=5066.0, P99=70.1ms (51%成功)
  → 连接池解决了崩溃问题，多线程可并行查询 MySQL

核心发现:
- c≤200 时连接池无帮助，瓶颈是 MySQL 查询延迟（~44ms）
- c=500 时连接池关键：8个连接并行处理，不再崩溃
- P99 在 c=500 时升到 70ms，说明高并发下有排队
- 下一步: Redis 缓存减少 MySQL 查询次数，预期 c=50 时 QPS 也能提升
```
