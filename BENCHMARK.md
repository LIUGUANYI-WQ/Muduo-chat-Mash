...
c=50:
  原始单连接:     QPS=1126.1, P99=48.7ms
  连接池8连接:    QPS=1126.1, P99=49.3ms
  连接池16连接:   QPS=1130.8, P99=48.3ms
  → 低并发无差异，瓶颈在 MySQL 单次查询延迟（44ms）

c=200:
  原始单连接:     QPS=4380.6, P99=51.5ms
...
c=500:
  ...
  → 16连接比8连接提升23%，P99降低22%

核心发现:
- 低并发(c≤200): 连接池无帮助，瓶颈是 MySQL 查询时间
- 高并发(c=500): 16连接 > 8连接，更多并行查询能力
- 预注册用户: 登录成功后返回 token，更接近真实场景
- 下一步: Redis 缓存已有用户登录，预期 QPS 大幅提升
```
