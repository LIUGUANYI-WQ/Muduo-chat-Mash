...
### 4.1 单 Reactor 模式（1个线程）

修改方式：main.cc 中 `server.setThreadNum(0);`，重新编译。
...
```
多 Reactor (setThreadNum(4)):
- M-1: c=50,  QPS=1129.5, P99=48.7ms  ✅
- M-2: c=200, QPS=4272.1, P99=53.9ms  ✅
- M-3: c=500, QPS=5027.4, P99=55.8ms  ⚠️ 30%连接失败

单 Reactor:
- 待测试（设置 setThreadNum(0)）

瓶颈分析:
- P50≈P99，延迟稳定，无长尾问题
- 延迟主要构成: MySQL查询(含SHA256) ≈ 30ms + 网络往返 ≈ 10ms + 服务端处理 ≈ 5ms
- M-3 崩溃原因: MySQL 单连接 + mutex 串行化，500并发时排队超时

下一步优化:
1. MySQL 连接池 — 解决 mutex 串行化瓶颈
2. Redis 缓存 — 热点用户登录走 Redis，减少 MySQL 查询
```
